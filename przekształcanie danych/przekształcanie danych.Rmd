...
Do bezpośedniej zmiany służy funcja rename. Schemat zmiany anzwy jak w przypadku select.
...
urodzeni_w_czasie_wś <- filter(panteon, birthyear %in% c(1914:1918, 1939:1945))

```

```{r}
table(urodzeni_w_czasie_wś$birthyear)
```
...
```{r}
library(missForest)
imputed_starwars <- starwars %>%
  mutate(mass = missForest(mass))
```
