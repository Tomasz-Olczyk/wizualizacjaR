...
p <- ggplot(m, aes(displ, hwy)) + geom_point() #mamy wykrs w ggplocie z tej nowej ramki
 
gg <- highlight(ggplotly(p), "plotly_hover") #mamy highlight(ggploly(tego wykresy)) i jeszcze argument dotyczący ego jak wybierać kolumny
...
Spróbujemy skonstruować animację ostatnich pięciu wiek lat pozycji Państw jeśli chodzi o liczbę osób sławnych ich sumaryczne HPI i sumaryczne LPstar
...
To poniżej nie działa tak jak powonno]
...
p	 <- plot_ly(g) %>% 
  group_by(country) %>% 
  add_lines(x = ~year, y = ~lifeExp, color = ~continent) %>% layout(xaxis = list(title = "")) %>% 
